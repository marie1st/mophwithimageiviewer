  ...
  <div className={`${styles.columns_is_centered}`}>
    ...
    <tr>
        ...
        {Customerls.filter(Customer=>Customer.created_at > prior_start).map((row, index) => (
          ...
          <td width="200"><Button color={"b"+`${row.status}`}>STATUS</Button><Button color={"b"+`${row.status_covid}`}>COVID-19</Button><Button color={"b"+`${row.status_fit}`}>FIT-TO-FLY</Button></td>
       </tr> 
        ))}
    </table>
  </div>
